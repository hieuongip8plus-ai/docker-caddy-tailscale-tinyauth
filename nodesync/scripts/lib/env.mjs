...
export function loadConfig(){
 ...
 const paths=process.env.SSH_SYNC_PATHS;
 if(paths!==undefined)cfg.sync_paths=String(paths).split(",").map(s=>s.trim()).filter(Boolean);
 else if(truthy(process.env.SSH_SYNC_SMOKE_ENABLE))cfg.sync_paths=["ci-runtime/smoke-sync-data"];
 return cfg;
}
...
export function collectSshUsers(env=process.env){
 const ids=new Set();for(const k of Object.keys(env)){const m=k.match(/^SSH_(\d+)_(?:USER|PASS|PASSWORD|PUBLIC_KEY_BASE64|PRIVATE_KEY_BASE64)/i);if(m)ids.add(Number(m[1]))}
 return [...ids].sort((a,b)=>a-b).map(index=>{const p=`SSH_${index}_`;return{index,user:env[`${p}USER`],password:env[`${p}PASS`]||env[`${p}PASSWORD`],publicKey:maybeB64(env[`${p}PUBLIC_KEY_BASE64`],true),privateKey:maybeB64(env[`${p}PRIVATE_KEY_BASE64`],true),uid:env[`${p}UID`],shell:env[`${p}SHELL`]||"/bin/bash",privileged:truthy(env[`${p}PRIVILEGED`],"1")}}).filter(x=>x.user);
}
